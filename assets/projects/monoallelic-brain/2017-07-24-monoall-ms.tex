...
We quantifieied allelic bias \added{based on RNA-seq reads} using a statistic called
...
\begin{itemize}
\item we studied how the allelic bias of the 30 imprinted genes we identified depends on certain biological variables
\item biol variables: Dx, Age, Gender and genetics (Ancestry)
\item to that aim we fitted several regression models to the data consisting of the
observed read count ratios (the response) and the corresponding observations
on biological and technical variables (the explanatory variables or predictors, Table~\ref{tab:predictors})
\item we used both mixed models that describe the variation of read count
ratio jointly for all 30 genes as well as sets of 30 conventional (i.e.~fixed effects)
regression models, one for each gene
\item the fitted models in both classes (mixed or separate gene-specific)
differed in three additional aspects Table~\ref{tab:model-names}
\begin{enumerate}
\item the transformation applied to the read count data
\item model family: the link function + error distribution
\item the terms of explanatory variables present in the linear predictor
\end{enumerate}
\end{itemize}
...
\begin{itemize}
\item we based our inference on mixed effects models due to their robustness
against overfitting, which we were concerned about given initial results based
on the separate (gene-specific) modeling strategy
\item we let preliminary results from the separate gene-wise modeling
framework to guide heuristic search for the best fitting mixed model
\item based on those results and on qualitative exploratory analysis we
guessed the set of most informative terms in order to
\begin{enumerate}
...
\end{enumerate}
\item fit during search was evaluated using information criteria AUC, BIC, as well as
p-value from Chi-squared test on the degrees of freedom that depends on the
term(s) added
\item the same criteria were used to infer the relevance of biological
variables by omitting each of them from the best model (Table~\ref{tab:mod-sel})
\end{itemize}
...
\begin{center}
\begin{tabular}{rlrc}
predictor term                              & interpretation: effect due to &   \(\Delta\)AIC &        p-value \\
\hline
\((1\,|\,\mathrm{Gene})\)                   & Gene, independently from other variables & \(-126.8\)  & \(8.5\times 10^{-28}\) \\
{\((1\,|\,\mathrm{Dx})\)}                   & Dx, independently from other variables & \(2.0\)  & \(1.0\)  \\
\((1\,|\,\mathrm{Dx}:\mathrm{Gene})\)       & interaction of Dx and Gene & \(0.4\) &  \(0.21\) \\
{\((1\,|\,\mathrm{Gender})\)}               & Gender, independently from other variables & \(2.0\)  & \(1.0\)  \\
\((1\,|\,\mathrm{Gender}:\mathrm{Gene})\)   & interaction of Gender and Gene & \(-5.7\) &  \(5.5\times 10^{-3}\) \\
\(\mathrm{Age}\)                            & Age, independently from other variables & underway   & underway \\
\((\mathrm{Age}\,|\,\mathrm{Gene})\)        & Age, depending on Gene & \(-18.9\)  & \(2.5\times 10^{-5}\)  \\
\(\mathrm{Ancestry.1}\)                     & Ancestry.1, independently from other variables & underway  & underway \\
\((\mathrm{Ancestry.1}\,|\,\mathrm{Gene})\) & Ancestry.1, depending on Gene & \(-71.2\)   & \(4.6\times 10^{-16}\) \\
\end{tabular}
\end{center}
\caption{Biological terms and their impact on the fit of the mixed model.  The
...
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.6]{figures/2016-11-01-plotting-distribution-of-s/S-Dx-density-1.png}
\caption{
Distribution of read count ratio for control and schizophrenic (SCZ)
individuals within each gene.
}
\label{fig:S-Dx}
\end{center}
\end{figure}
